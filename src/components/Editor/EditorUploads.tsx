...
export const EditorUploads: FC<IEditorUploads> = () => {
        ...
        axios
            ...
            .then((res) => {
                ...
                res.data.forEach((nameFile, idx) => {
                    uploadsList.push({
                        name: nameFile,
                        path: `${path}upload_image/${nameFile}`,
                        isUsed: false,
                        id: idx,
                    })
                })
                ...
                const mapList = uploadsList.map((item) => {
                    const profile: any = images.find((img) =>
                        img.src.includes(item.name)
                    )
    ...
    const deleteImage = (img: IUploadsImages) => {
        const newSrc =
            import.meta.env.MODE === 'development' ? img.path : `../${img.path}`

        axios
            .post(`${pathAPI}deletingImage.php`, { arraySrc: [newSrc] })
            .then(() => {
                const filtered = uploadsImages.filter(
                    (item) => item.id !== img.id
                )
    ...
    return (
        <div className="space-y-4">
            {filteredImages.length > 0 && notUsedList().length > 0 && (
                <div className="bg-red-200 dark:text-gray-900 p-4 rounded-lg flex item-center justify-between flex-wrap">
            ...
            <div className="bg-amber-100 dark:text-gray-900 p-4 rounded-lg">
                {t('imagesStored')} "./upload_image"
            </div>
            <div className="flex items-center justify-between">
                <Select
                    array={[t('all'), t('used'), t('notUsed')]}
                    setSelect={filter}
                />
                <p>
